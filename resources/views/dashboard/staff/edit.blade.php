...
<div class="col-lg-8">
  <form method="post" action="{{ url('/dashboard/staff/' . $staff->id) }}" class="mb-5">
    @method('put')
    @csrf
    <div class="mb-3">
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $staff->email) }}" required>
      @error('email')        
      <div class="invalid-feedback">
        {{ $message }}
      </div>
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control @error('telp') is-invalid @enderror" id="telp" name="telp" value="{{ old('telp', $staff->telp) }}">
      @error('telp')        
      <div class="invalid-feedback">
        {{ $message }}
      </div>
    ...
    <div class="mb-3">
      ...
      <select class="form-select @error('komponen_id') is-invalid @enderror" name="komponen_id" required>
        ...
        @foreach ($komponens as $komponen)
          @if (old('komponen_id', $staff->komponen_id) == $komponen->id)
